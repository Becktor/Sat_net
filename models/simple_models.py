...
class SimpleNetwork(nn.Module):
    def __init__(self, input_size):
        ...
        self.loss = QuantileLoss(
            quantiles=self.quantiles)  # nn.MSELoss(size_average=None, reduce=None, reduction='mean')
...
class AutoEncoderNetwork(nn.Module):
    def __init__(self, input_size):
        ...
        # here we add the individual layers
        self.input_size = input_size
        self.angles_to_image = nn.Linear(3, input_size * input_size)
        self.relu = nn.ReLU()
        self.conv1 =nn.Conv2d(4, 32, kernel_size=(3, 3), padding=(1, 1))
        self.max_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.bilinear = nn.UpsamplingBilinear2d(scale_factor=2)
        self.bilinear_to_size = nn.UpsamplingBilinear2d(size=input_size)
        self.pi = torch.tensor(np.pi)
        ...
        self.dec3 = ConvBlock(256, 64)
        self.dec4 = ConvBlock(128, 32)
        self.dec5 = ConvBlock(64, 5)

        #self.final_conv = nn.Conv2d(64, 5, kernel_size=(3, 3), padding=(1, 1))
        self.loss = nn.MSELoss(size_average=None, reduce=None, reduction='mean')

    def forward(self, x, ang, y) -> torch.tensor:
        ang = self.relu(self.angles_to_image(ang).reshape(-1, 1, x.shape[2], x.shape[3]))
        x = torch.cat([x, ang], 1)
        skip0 = self.relu(self.conv1(x))
        skip1 = self.max_pool(self.enc1(skip0))
        skip2 = self.max_pool(self.enc2(skip1))
        skip3 = self.max_pool(self.enc3(skip2))
        x = self.max_pool(self.enc4(skip3))

        x = self.bilinear(self.dec1(x))
        joined_x = torch.cat([x, skip3], 1)
        x = self.bilinear(self.dec2(joined_x))
        joined_x = torch.cat([x, skip2], 1)
        x = self.bilinear(self.dec3(joined_x))
        joined_x = torch.cat([x, skip1], 1)
        x = self.bilinear(self.dec4(joined_x))
        joined_x = torch.cat([x, skip0], 1)
        output = self.dec5(joined_x)
